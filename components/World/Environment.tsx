...
const StarField: React.FC = () => {
  ...
  const { positions, speedFactors } = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Parallax Factor: 0.1 (far/slow) to 1.5 (close/fast)
      // This creates distinct depth layers
      factors[i] = 0.1 + Math.random() * 1.4;
    ...
    return { positions: pos, speedFactors: factors };
  }, [count]);
  ...
  useFrame((state, delta) => {
    ...
    for (let i = 0; i < count; i++) {
        ...
        const factor = speedFactors[i];
        
        // Apply varied speed based on the parallax factor
        z += activeSpeed * delta * 2.0 * factor;
